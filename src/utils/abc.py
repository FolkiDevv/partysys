...
class ServerABC(abc.ABC):
    ...
    def __init__(self, bot: "BotABC", guild: discord.Guild):
        ...
        self._creator_channels: MappingProxyType[int, CreatorChannels] = (
            MappingProxyType({})
        )
        ...
        self._last_data_update = datetime.datetime.fromtimestamp(0.0)
    ...
    @classmethod
    @final
    async def new(cls, bot: "BotABC", guild: discord.Guild) -> Self:
        instance = super().__new__(cls)
        instance.__init__(bot, guild)

        await instance._update_settings(guild.id)
    ...
    @abc.abstractmethod
    async def del_tv(self, channel_id: int) -> None: ...

    @abc.abstractmethod
    def is_creator_channel(self, channel_id: int) -> bool: ...

    @abc.abstractmethod
    def is_tv_channel(self, channel_id: int) -> bool: ...

    @abc.abstractmethod
    def get_member_tv(
        self, member: discord.Member, interaction_channel_id: int | None = None
    ...
    @abc.abstractmethod
    def tv(self, channel_id: int) -> TempVoiceABC | None: ...

    @abc.abstractmethod
    def all_tv(self) -> dict[int, TempVoiceABC]: ...
...
class BotABC(abc.ABC, commands.Bot):
    ...
    @abc.abstractmethod
    async def server(self, guild_id: int) -> ServerABC | None: ...
