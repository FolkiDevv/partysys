...
class ServerTempVoices:
    def __init__(self, bot: services.PartySysBot, guild: discord.Guild):
        ...
        self._get_settings_from_db(guild.id)
    ...
    def _get_settings_from_db(self, guild_id: int) -> None:
        self.bot.cur.execute(
            "SELECT * FROM servers WHERE dis_id=%s LIMIT 1", (guild_id,)
        )
        if server := self.bot.cur.fetchone():
        ...
        self._creator_channels = MappingProxyType(
            {channel["dis_id"]: channel for channel in self.bot.cur.fetchall()}
        )
    ...
    async def create_channel(
        ...
        self.bot.cur.execute(
            "INSERT INTO temp_channels (dis_id, dis_creator_id, dis_owner_id) "
            "VALUES (%s, %s, %s)",
        ...
        self.bot.cur.execute(
            "SELECT dis_banned_id FROM ban_list WHERE server_id=%s AND "
            "dis_creator_id=%s AND banned=1",
            (self.server_id, member.id),
        )
        for raw_ban in self.bot.cur.fetchall():
    ...
    def channel(self, channel_id: int) -> services.TempVoice | None:
        ...
        if (
            ...
            self._get_settings_from_db(self.guild.id)
        ...
        if adv_msg_id:
            try:
                if adv_msg := await self.adv_channel.fetch_message(adv_msg_id):
                    self._temp_channels[channel.id].adv = services.Adv(adv_msg)
